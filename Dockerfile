# ─────────────────────────────────────────────────────────────────────────────
# polish-cybersecurity-mcp — multi-stage Dockerfile
# ─────────────────────────────────────────────────────────────────────────────
# Build:  docker build -t polish-cybersecurity-mcp .
# Run:    docker run --rm -p 3000:3000 polish-cybersecurity-mcp
#
# The image bakes /app/data/cert_pl.db at build time. Override
# CERT_PL_DB_PATH for a custom location at runtime.
# ─────────────────────────────────────────────────────────────────────────────
...
ENV CERT_PL_DB_PATH=/app/data/cert_pl.db
...
# Bake the pre-built database into the image so /app/data/cert_pl.db resolves
...
# We then COPY it into the image at /app/data/cert_pl.db (CERT_PL_DB_PATH).
# The explicit `data/<name>.db` reference is required for the workflow's grep
# `COPY\s+\K(data/\S+\.db)` to match (Case 1/2 path).
COPY data/database.db data/cert_pl.db
